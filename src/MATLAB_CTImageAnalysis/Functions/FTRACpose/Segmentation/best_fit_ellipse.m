% $Id: best_fit_ellipse.m,v 1.2 2005/11/07 00:05:11 svikal Exp $

function [ellipse_t, el_eq] = best_fit_ellipse( x,y,axis_handle, bUseCallback )
%
...
%     plot( new_horz_line(1,:),new_horz_line(2,:),'c' );
    if(bUseCallback)
        plot( rotated_ellipse(1,:),rotated_ellipse(2,:),'c' ,'ButtonDownFcn', 'ImageAxesClick_Callback' );
    else
        plot( rotated_ellipse(1,:),rotated_ellipse(2,:),'c');
    end
    set( axis_handle,'NextPlot',hold_state);
end


% $Log: best_fit_ellipse.m,v $
% Revision 1.2  2005/11/07 00:05:11  svikal
% integrating with FTRAC;output of segmentation in desired format of FTRAC
%
% Revision 1.1  2005/11/05 22:56:12  svikal
