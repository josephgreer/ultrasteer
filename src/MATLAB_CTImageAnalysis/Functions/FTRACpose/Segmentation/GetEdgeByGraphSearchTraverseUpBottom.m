% $Id: GetEdgeByGraphSearchTraverseUpBottom.m,v 1.1 2005/10/27 19:10:47 svikal Exp $
...
%               Node2;
%               Node3......]
function [lEdge] = GetEdgeByGraphSearchTraverseUpBottom (IedgeImage, iEdgeThreshold, pStart, pEnd, bUseGlobalXInfo, bUseDirectionEstimate, InitialDirectionEstimate)
...
%   connected node
	...
	while(~	(  ((bUseGlobalXInfo) && (nodeToExpand(1,1) == pEnd(1,1)) && (nodeToExpand(1,2) == pEnd(1,2))) || ((~bUseGlobalXInfo) && (nodeToExpand(1,1) == pEnd(1,1)))	) )
...
% 		//x+1,y-1
		if ((iX_Index + 1) < iX_Dimension && (iY_Index >1))
...
% 			//first look back at last 10 points to get an idea
			...
			while( (nodeToLookBack(1,3) ~= 0) && i < 10)
				...
				nodeToLookBack = lOpenPointsList (nodeToLookBack(1,3),:);
...
% $Log: GetEdgeByGraphSearchTraverseUpBottom.m,v $
