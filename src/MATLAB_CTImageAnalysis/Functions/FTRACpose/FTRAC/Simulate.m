...
function [net_err, pose_err]= Simulate(fid_depth)
...
while (pose_acceptable==0)   
    actual_pose = cone_pose(cone_angle, 2*pi*rand(1), (pi/18)*rand(1)*sign(rand(1)-0.5), [0 0 -fid_depth]' + [50 ; 50 ; 25].*rand(3,1).*sign((rand(3,1)-0.5)));
...
%guess, camera parameters and segmentaed data.
initial_guess = actual_pose*xyz_xform(0.5*pi*(rand(3,1).*sign((rand(3,1)-0.5))), 100*rand(3,1).*sign((rand(3,1)-0.5)));
[pose, pose_err] = Estimate_Pose(model_pts, image_pts, model_line_pts, image_lines, model_ellipse1_pts, model_ellipse2_pts, image_ellipses, pixel_ratio, screen_size, origin, focal_len, initial_guess);
...
%    net_err = [err_tras' err_rota];
    err_tras = abs(actual_pose(1:3,4) - pose(1:3,4));
    err_rota = acos(sum(actual_pose(1:3,1:3).*pose(1:3,1:3)))*180/pi;
    net_err = [err_tras' err_rota];
end