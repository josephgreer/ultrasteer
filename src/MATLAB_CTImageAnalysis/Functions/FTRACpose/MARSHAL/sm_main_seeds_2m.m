function [pts_3d,r,res_ra,res_ra_m,res_ra_u,res_bpe,res_bpe_m,res_bpe_u,res_re,res_re_m,res_re_u,res_ren,res_ren_m,res_ren_u]=sm_main_seeds_2m(f_dir,i_comb)
...
[M,cost]=sm_matching_2im(pts_2d,trs,ex,ey,xo,yo,f);
[r,matched,unmatched,m_truth]=sm_matchingrate_2im(M);
[n_m,tmp]=size(matched);
[n_u,tmp]=size(unmatched);
[n,tmp]=size(M);
M=[matched;unmatched];
[pts_3d,ra]=sm_recon_general_2({pts_2d{1}(M(:,1),:);pts_2d{2}(M(:,2),:)},trs,ex,ey,xo,yo,f);
ra_mean=mean(ra);
ra_std=std(ra);
ra_min=min(ra);
ra_max=max(ra);
