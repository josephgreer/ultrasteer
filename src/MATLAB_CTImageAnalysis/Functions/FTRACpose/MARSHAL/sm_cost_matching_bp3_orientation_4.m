function tab_c=sm_cost_matching_bp3_orientation_4(pts_2d,ors,trs,ex,ey,xo,yo,f)
...
errs=(p2dtmp(1,:).*p2dtmp(1,:)+p2dtmp(2,:).*p2dtmp(2,:))';

%end error;
aa=repmat(pt2d1,1,n_seeds)';
bb=repmat(reshape(ors{3}',2*n_seeds,1),1,n_seeds*n_seeds);
%
d0=sm_dist_ptline(reshape(a,2,n_seeds^3)',reshape(bb,2,n_seeds^3)',reshape(b,2,n_seeds^3)');
tmp0=reshape(d0,n_seeds,n_seeds*n_seeds);
dh=sqrt(errs-d0.^2);
tmp=reshape(dh,n_seeds,n_seeds*n_seeds);
...
for k=1:n_ims
    ...
    p2dtmp=pt2d-pt_2d{k};
    errs=p2dtmp(:,1).*p2dtmp(:,1)+p2dtmp(:,2).*p2dtmp(:,2);
    d0=sm_dist_ptline(pt2d,os_2d{k},pt_2d{k});
