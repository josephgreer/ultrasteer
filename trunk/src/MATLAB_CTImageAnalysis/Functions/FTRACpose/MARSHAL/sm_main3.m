...
%pts_2d0=pts_2d_all([1,2,5,4]);
%err=4;
%pts_2d={add_error(pts_2d0{1},err);add_error(pts_2d0{2},err);add_error(pts_2d0{3},err);add_error(pts_2d0{4},err)};
pts_2d{1,1}=dlmread('pts2dnew1.txt','\t');
pts_2d{2,1}=dlmread('pts2dnew2.txt','\t');
pts_2d{3,1}=dlmread('pts2dnew3.txt','\t');
pts_2d{4,1}=dlmread('pts2dnew4.txt','\t');
trs=trs_all([1,2,5,4]);
...
c=cell(3,1);
%t0=clock;
c{1}=sm_cost_matching_bp4_new(pts_2d,trs,ex,ey,xo,yo,f);
%etime(t0,clock)
c{2}=sm_cost_matching_bp4_new({pts_2d{2};pts_2d{3};pts_2d{1};pts_2d{4}},{trs{2};trs{3};trs{1};trs{4}},ex,ey,xo,yo,f);
c{3}=sm_cost_matching_bp4_new({pts_2d{3};pts_2d{1};pts_2d{2};pts_2d{4}},{trs{3};trs{1};trs{2};trs{4}},ex,ey,xo,yo,f);
[fff,cost]=sm_matching_3im2(pts_2d(1:3),trs,c,ex,ey,xo,yo,f);
pt_new=sm_reorg_pix(pts_2d(1:3),fff(:,1:3));
[r,matched,unmatched,m_truth]=sm_matchingrate2(fff);
[p,ds]=sm_recon(pt_new,trs(1:3),fff(:,4),ex,ey,xo,yo,f);
...
[p_m,ds]=sm_recon({pt_new{1}(matched(1:n_m,1),:);pt_new{2}(matched(1:n_m,1),:);pt_new{3}(matched(1:n_m,1),:)},trs(1:3),fff(matched(1:n_m,1),4),ex,ey,xo,yo,f);
...
if n_u>0
    ...
    [p_u,ds]=sm_recon({pt_new{1}(unmatched(1:n_u,1),:);pt_new{2}(unmatched(1:n_u,1),:);pt_new{3}(unmatched(1:n_u,1),:)},trs(1:3),fff(unmatched(1:n_u,1),4),ex,ey,xo,yo,f);
