function [pts_3d,r,res_ra,res_ra_m,res_ra_u,res_bpe,res_bpe_m,res_bpe_u,res_re,res_re_m,res_re_u,res_ren,res_ren_m,res_ren_u]=sm_main_seeds_2m1(f_dir,i_comb)
...
for i=1:3
    ...
    yo(i,1)=cams{i,1}(4);
    f(i,1)=cams{i,1}(5);
%    ex(i,1)=0.25;%cams{i,1}(1);
%    ey(i,1)=0.25;%cams{i,1}(2);
%    xo(i,1)=600;%cams{i,1}(3);
%    yo(i,1)=600;%cams{i,1}(4);
%    f(i,1)=1000;%cams{i,1}(5);
end

[M,cost]=sm_matching_2im1(pts_2d,trs,ex,ey,xo,yo,f);
[r,matched,unmatched,m_truth]=sm_matchingrate_2im(M);
[n_m,tmp]=size(matched);
[n_u,tmp]=size(unmatched);
[n,tmp]=size(M);
M=[matched;unmatched];
[pts_3d,ra]=sm_recon_general_2({pts_2d{1}(M(:,1),:);pts_2d{2}(M(:,2),:)},trs(1:2),ex(1:2),ey(1:2),xo(1:2),yo(1:2),f(1:2));
ra_mean=mean(ra);
ra_std=std(ra);
ra_min=min(ra);
ra_max=max(ra);
