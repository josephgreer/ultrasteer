function tab_c=sm_cost_matching_bp6_new2(pts_2d,trs,ex,ey,xo,yo,f)
...
[pt_3d,d]=sm_recon_general_2(pt_2d,trs(1:4),ex(1:4),ey(1:4),xo(1:4),yo(1:4),f(1:4));
%backproject to 5;
pt2d=sm_backproj_general(pt_3d,trs{5},ex(5),ey(5),xo(5),yo(5),f(5));
%find the smallest error in 5;
a=repmat(pt2d,1,n_seeds)';
b=repmat(reshape(pts_2d{5}',2*n_seeds,1),1,n_seeds*n_seeds);
p2dtmp=reshape(a-b,2,n_seeds*n_seeds*n_seeds);
errs=sqrt(p2dtmp(1,:).*p2dtmp(1,:)+p2dtmp(2,:).*p2dtmp(2,:));
tmp=reshape(errs,n_seeds,n_seeds*n_seeds);
[vtmp,ind]=min(tmp);
...
[pt_3d,d]=sm_recon_general_2(pt_2d,trs(1:5),ex(1:5),ey(1:5),xo(1:5),yo(1:5),f(1:5));
%backproject to 6;
pt2d=sm_backproj_general(pt_3d,trs{6},ex(6),ey(6),xo(6),yo(6),f(6));
%find the smallest error in 6;
a=repmat(pt2d,1,n_seeds)';
b=repmat(reshape(pts_2d{6}',2*n_seeds,1),1,n_seeds*n_seeds);
p2dtmp=reshape(a-b,2,n_seeds*n_seeds*n_seeds);
errs=sqrt(p2dtmp(1,:).*p2dtmp(1,:)+p2dtmp(2,:).*p2dtmp(2,:));
tmp=reshape(errs,n_seeds,n_seeds*n_seeds);
[vtmp,ind]=min(tmp);
