function tab_c=sm_cost_matching_bp5_new(pts_2d,trs,ex,ey,xo,yo,f)
...
ind1=reshape(repmat((1:n_seeds)',1,n_seeds)',1,n_seeds*n_seeds)';
pt2=repmat(pts_2d{2},n_seeds,1);
ind2=repmat((1:n_seeds)',n_seeds,1);
...
for k=1:n_ims
    ...
    pt2dm=reshape(repmat(pt2d,1,n_seeds)',2,n_seeds*n_seeds*n_seeds)';
    p2dtmp=pt2dm-repmat(pts_2d{k},n_seeds*n_seeds,1);
    errs=sqrt(p2dtmp(:,1).*p2dtmp(:,1)+p2dtmp(:,2).*p2dtmp(:,2));
    tmp=reshape(errs,n_seeds,n_seeds*n_seeds);
    tp1=sum(tmp)';
    
    pt2dm=reshape(repmat(pts_2d{k}(indm(:,k),:),1,n_seeds)',2,n_seeds*n_seeds*n_seeds)';
    p2dtmp=pt2dm-repmat(pts_2d{k},n_seeds*n_seeds,1);
    errs=sqrt(p2dtmp(:,1).*p2dtmp(:,1)+p2dtmp(:,2).*p2dtmp(:,2));
    tmp=reshape(errs,n_seeds,n_seeds*n_seeds);
    tp2=sum(tmp)';
