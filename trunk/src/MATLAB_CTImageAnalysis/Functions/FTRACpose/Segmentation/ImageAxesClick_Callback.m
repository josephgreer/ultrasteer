% $Id: ImageAxesClick_Callback.m,v 1.10 2007/01/08 16:47:37 anton Exp $
...
%       add a seed
        ...
        if (SearchClosestPoint == 1)
            PointToAdd = LocateDarkestPoint (I(seedROIStartX:seedROIStartX + seedROIXWidth, seedROIStartY:seedROIStartY + seedROIYWidth), [YToAdd XToAdd]);
        else
            PointToAdd = [YToAdd XToAdd];
        end
        % final data set: [CGx CGy Orientation OrientationValidity PartOfCluster]
        FloatingSeedSegmentationFinalInformationDataSet = [FloatingSeedSegmentationFinalInformationDataSet; PointToAdd(1,1) PointToAdd(1,2) 0 0 0];
        
        SeedUndoTaskList = [1 PointToAdd(1,2) PointToAdd(1,1);SeedUndoTaskList];
...
%       delete/remove a seed  
        ...
        if (iSeedIndex > 0)
            DelCG = FloatingSeedSegmentationFinalInformationDataSet (iSeedIndex, 1:2);

            FloatingSeedSegmentationFinalInformationDataSet (iSeedIndex, :) = [];    
            
            SeedUndoTaskList = [2 DelCG(1,2) DelCG(1,1);SeedUndoTaskList];
...
%       add BB on Fiducial
        ...
        if ( find(Floating_BBLocations(:,1) == -1) )
            ...
            PointToAdd = LocateDarkestPoint (I(FiducialROIStartX:FiducialROIStartX + FiducialROIXWidth, FiducialROIStartY:FiducialROIStartY + FiducialROIYWidth), [YToAdd XToAdd], 3);
            
            InsertIndex = find (Floating_BBLocations (:,1) == -1);
        
            Floating_BBLocations(InsertIndex(1),:) = PointToAdd;
        
            FiducialUndoTaskList = [3 PointToAdd(1,2) PointToAdd(1,1); FiducialUndoTaskList];
...
%       delete BB on Fiducial  
        ...
        if (iBBIndex > 0)
            ...
            FiducialUndoTaskList = [4 DelBB(1,2) DelBB(1,1); FiducialUndoTaskList];
...
%         
                    if(bLine1Removed)
                        ...
                        bLine1Removed = false;
                        FiducialUndoTaskList = [5 1 0; FiducialUndoTaskList];
                        
                    else 
                        if(bLine2Removed)
                            Floating_lLine2PointsEq = least_squares_line(PointsOnLine);
                            bLine2Removed = false;
                            FiducialUndoTaskList = [5 2 0; FiducialUndoTaskList];
                        else 
                            if(bLine3Removed )
                                Floating_lLine3PointsEq = least_squares_line(PointsOnLine);
                                bLine3Removed = false;
                                FiducialUndoTaskList = [5 3 0; FiducialUndoTaskList];
                            end
                        end
...
%       delete a line
       ...
       if (bOperationCompleted)
           DisplayFiducialSegmentationROI;
           FiducialUndoTaskList = [6 LineDeleted 0; FiducialUndoTaskList];
       else
          %   alert user that no seed in close vicinity found, try by getting closer to the seed you wish to delete
...
%              remove ellipse 1
               ...
               FiducialUndoTaskList = [8 1 0; FiducialUndoTaskList];
           else
%              remove ellipse 2  
               bEllipse2Removed = true;
               Floating_lEllipse2Points = [];
               FiducialUndoTaskList = [8 2 0; FiducialUndoTaskList];
           end
...
% Revision 1.10  2007/01/08 16:47:37  anton
% Segmentation:  Fixed older callback to define a search area for new definition
% of LocateDarkestPoint().
%
