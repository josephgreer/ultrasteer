...
% BREAK_DOWN_LOOPS - handles loops too large for brute force
%   This function creates a network flow for the matching loop that is too
%   large for the brute force to handle.  This function is recursive when
%   larger loops arise again.
...
[matched, loops, mis_matched_seeds, bpe] = find_loops(img_tot, cost, pts_2d, pts_2d_hidden, trs, ex, ey, xo, yo, focal);
...
while (~isempty(mis_matched_seeds))
    ...
    if (~isempty(matched_sub))
        matched = [[matched(:,1:3) -1*ones(size(matched, 1), 1)]; [matched_sub -2*ones(size(matched_sub, 1), 1)]];
    end
    
    if (~isempty(loops_sub))
        matched_sub = resolve_loops_special(loops_sub, pts_2d, trs, ex, ey, xo, yo, total_seeds, focal);
        if (~isempty(matched_sub))
            matched = [[matched(:,1:3) -1*ones(size(matched, 1), 1)]; [matched_sub -3*ones(size(matched_sub, 1), 1)]];
        end
    end
end
...
if (~isempty(loops))
    ...
    if (~isempty(matched_sub))
        try
            matched = [[matched(:,1:3) -1*ones(size(matched, 1), 1)]; [matched_sub -4*ones(size(matched_sub, 1), 1)]];
        catch
