function h=BreakXAxis(x,y,start,stop,width,ymin,ymax)

% Julie Haas, after Michael Robbins and Ann Majewicz
...
x2=x;
space = double(stop-start-width);
x2(x2>=stop)=x2(x2>=stop)-space;

h=plot(x2,y,'k');

% ytick=get(gca,'YTick');
% t1=text(double(start+width/2),ytick(1),'//','fontsize',15);
% t2=text(double(start+width/2),ytick(max(length(ytick))),'//','fontsize',15);

t1=text(double(start),ymin,'//','fontsize',15);
t2=text(double(start),ymax,'//','fontsize',15);
