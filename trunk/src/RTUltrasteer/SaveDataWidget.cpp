...
#define MAX_TIMESTEPS 800
  ...
  void SaveDataWidget::SaveData(RPFileHeader header)
  {
    ...
    if(m_dataToSave.size() > 0 && m_dataToSave.front().rp.color != NULL)
      rpw->AddWriter(RPF_BPOST32);
    rpw->AddGPSWriter();
    rpw->AddGPSWriter2();

    
    std::string path = std::string(dir+std::string("/")+name+std::string(".u"));
    FILE *f = fopen(path.c_str(), "wb");

    for(s32 i=0; i<m_dataToSave.size(); i++) {
      rpw->WriteNextRPData(&m_dataToSave[i].rp);
